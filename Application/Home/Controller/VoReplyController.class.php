...
class VoReplyController extends CommonController
{
    public function index(){
        ...
        $count = $voreply->where($map)->count();
        $Page=getpage($count);
        $show=$Page->show();//分页显示输出
        $this->assign('map',$map);
        $list = $voreply->order('id desc')->where($map)->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function add(){
        ...
        if (IS_POST) {
            ...
            if ($voreply -> add($data)) {
                header('location:'.U('VoReply/index'));
                return;
            } else {
                $this -> error('操作失败！');
                return;
    ...
    public function  update(){
        ...
        $id=I('id');
        $voreply=M('voreply');
        $detail=$voreply->where("id='".$id."'")->find();
