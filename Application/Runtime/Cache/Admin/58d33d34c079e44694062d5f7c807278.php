	...
	<form action="<?php echo U('Menue/add');?>" method="post" class="form form-horizontal" id="form-member-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-3 col-sm-2">
				<input type="text" class="input-text" value="<?php echo ($name); ?>" placeholder="" id="menu_cate" name="menu_cate">
			</div>
		</div>
		<div class="row cl">
			<button type="button" onclick="add()" class="btn btn-primary">添加下级菜单</button>
		</div>
		<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="delmen"><label class="form-label col-xs-3 col-sm-2"><span class="c-red">*</span>二级级菜单名称：</label>
			<div class="formControls col-xs-3 col-sm-2"> <input type="text" class="input-text" value="<?php echo ($vo["menu_tite"]); ?>" placeholder=""  name="title_name[<?php echo ($k); ?>]"> </div>
			<label class="form-label col-xs-3 col-sm-2"><span class="c-red">*</span>URL：</label>
			<div class="formControls col-xs-3 col-sm-2"><input type="text" class="input-text" value="<?php echo ($vo["titel_code"]); ?>" placeholder=""  name="title_code[<?php echo ($k); ?>]"></div>
			<div class="row cl">
				<button type="button" onclick="adds('<?php echo ($k); ?>')" class="btn btn-primary radius">添加按钮</button><button type="button" onclick="delss(this)" class="btn btn-warning radius">删除二级</button></div>
			<div id="menubutton<?php echo ($k); ?>">
			<?php if(is_array($vo["buttons"])): $ke = 0; $__LIST__ = $vo["buttons"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$detail): $mod = ($ke % 2 );++$ke;?><div class="meunes"><label class="form-label col-xs-3 col-sm-2"><span class="c-red">*</span>按钮：</label><div class="formControls col-xs-4 col-sm-3">
				<input type="text" class="input-text" value="<?php echo ($detail["menu_name"]); ?>" placeholder=""  name="menu_name[<?php echo ($ke); ?>][<?php echo ($k); ?>]"></div><label class="form-label col-xs-3 col-sm-2">
					<span class="c-red">*</span>URL：</label><div class="formControls col-xs-4 col-sm-3">
					<input type="text" class="input-text" value="<?php echo ($detail["menu_code"]); ?>" placeholder=""  name="menu_code[<?php echo ($ke); ?>][<?php echo ($k); ?>]"></div>
				<div class="row cl"><button type="button" onclick="del(this)" class="btn btn-danger radius">删除</button></div></div><?php endforeach; endif; else: echo "" ;endif; ?>
