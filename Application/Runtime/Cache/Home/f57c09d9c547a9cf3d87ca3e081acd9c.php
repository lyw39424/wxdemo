...
<form action="<?php echo U('ImgReply/index');?>" method="get" class="form form-horizontal" id="form-member-add">
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="picture_add('添加图片','/wxdemo/ImgReply/add')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i>添加图片</a></span> <span class="r"><?php echo ($page); ?></span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td >   <a style="text-decoration:none" class="ml-5" onClick="picture_edit('文本编辑','/wxdemo/Home/ImgReply/update','<?php echo ($vo["id"]); ?>')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			url: '/wxdemo/Home/ImgReply/delete',
			type: "post",
			data: {
				'id': id
			},
