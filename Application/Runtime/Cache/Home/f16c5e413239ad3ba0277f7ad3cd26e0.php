...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 关键字 <span class="c-gray en">&gt;</span> 关键字列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<form action="<?php echo U('KeyReply/index');?>" method="get" class="form form-horizontal" id="form-member-add">
<div class="page-container">
	<input type="text" name="code" id="code" placeholder=" 标题名称" value="<?php echo ($map["key_name"]); ?>" style="width:250px" class="input-text">
		<button class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜标题</button>
	</div>
</form>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="picture_add('添加关键字','/wxdemo/KeyReply/add')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加关键字</a></span> <span class="r"><?php echo ($page); ?></span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<td><?php echo ($vo["key_name"]); ?></td>
					<td><?php echo ($vo["key_type"]); ?></td>
					<td ><?php echo ($vo["creat_time"]); ?></td>
					<td class="td-manage"> <a style="text-decoration:none" class="ml-5" onClick="picture_edit('文本编辑','/wxdemo/Home/KeyReply/update','<?php echo ($vo["id"]); ?>')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="picture_del(this,'<?php echo ($vo["id"]); ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			url: '/wxdemo/Home/KeyReply/delete',
			type: "post",
			data: {
				'id': id
			},
